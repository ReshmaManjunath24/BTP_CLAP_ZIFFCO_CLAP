...
define view entity ZDD_CUST_SALESAREA
  as select from ztab_sale_area
  
 ------Required if Composition Approch is used -----------
 --association  to parent ZDD_CUSTOMER as _zdd_clap on  $projection.zcustomer_num      = _zdd_clap.zcustomer_num
 --                                                 and $projection.zsales_orgnization = _zdd_clap.zsales_orgnization
  ...
  //association [0..*] to ZDD_CUSTOMER        as _customer on  $projection.zcustomer_num = _customer.zcustomer_num
{
  key zcustomer_num,
